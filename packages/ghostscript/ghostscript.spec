...
# --- Factory trims ------------------------------------------------------------
#
# libppd and cups-filters need gs, libgs and its cups raster device, and the
# publish gate installs them from Hummingbird plus this factory alone. Each
# switch below is off because what it pulls in at runtime is in neither, or
# because nothing in the Utah contract uses it.
#
# The Resource/ tree (the URW base 35 fonts, Adobe CMaps and
# DroidSansFallback) is used as upstream ships it. Fedora symlinks it to
# urw-base35-fonts, adobe-mappings-cmap, adobe-mappings-pdf and
# google-droid-sans-fonts and makes libgs Require all four; none is in
# Hummingbird, and importing them would add four data recipes to replace files
# the tarball already carries.
%bcond system_resources 0
# jbig2dec and ijs are built from the copies in the tarball. libjbig2dec.so.0
# and libijs-0.35.so are in neither Hummingbird nor this factory, and libgs
# checks jbig2dec's version at runtime anyway (see below), so the bundled copy
# is the one that always matches. JBIG2 images in PDFs and the ijs device
# both keep working.
%bcond system_jbig2dec_ijs 0
# libpaper only picks the default paper size from /etc/papersize, and
# libpaper.so.2 is in neither repository. CUPS passes the page size to gs
# explicitly, so printing does not need it.
%bcond libpaper 0
# The gtk viewer (gsx) and the X11 devices are for viewing a document in a
# window from the command line. Nothing in the Utah contract uses them, and
# they would make ghostscript, and with it libppd and cups-filters, wait on
# gtk3 and libXt in the build order. RHEL builds --without-x the same way.
%bcond gtk 0
%bcond x11 0
# dvipdf runs dvips, and texlive-dvips is in neither repository.
%bcond dvipdf 0
...
%if %{with system_resources} && %{with system_jbig2dec_ijs}
License:          AGPL-3.0-or-later
%else
# The bundled Droid font is Apache-2.0, the Adobe CMaps are BSD-3-Clause and
# ijs is MIT; the URW fonts and jbig2dec are AGPL-3.0-or-later like gs.
...
rm -rf cups/libs freetype jpeg lcms2* leptonica libpng openjpeg tesseract tiff windows zlib
%if %{with system_jbig2dec_ijs}
rm -rf ijs jbig2dec
%endif
...
# Without gtk, the build still installs gsx, as a second copy of the console
# loader gsc; and install-scripts installs dvipdf whether or not it is shipped.
%if %{without gtk}
rm -f %{buildroot}%{_bindir}/gsx
%endif
%if %{without dvipdf}
rm -f %{buildroot}%{_bindir}/dvipdf %{buildroot}%{_mandir}/man1/dvipdf.1*
%endif
